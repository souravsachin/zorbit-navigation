...
import { NavigationEventType, ZorbitEventEnvelope } from './navigation.events';
...
export class EventPublisherService implements OnModuleInit, OnModuleDestroy {
   ...
   * @param eventType - e.g. 'navigation.menu.updated'
  ...
  async publish<T>(
    ...
    const envelope: ZorbitEventEnvelope<T> = {
      ...
      source: 'zorbit-navigation',
      namespace,
      namespaceId,
      payload,
    };

    const topic = eventType.replace(/\./g, '-'); // navigation.menu.updated → navigation-menu-updated
